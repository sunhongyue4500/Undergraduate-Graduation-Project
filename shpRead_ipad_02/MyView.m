...
-(void)mouseMoved:(NSEvent *)theEvent
{
	...
	if(load)
	{
		...
		if(bInside)
		{
			...
			NSString* aString = [NSString stringWithFormat:@"East: %ld North: %ld",
...
-(void)displayZoomFactor
{
	...
	NSString* strZoom = [NSString stringWithFormat:@"%ld %%", (long)(roundtol(zoom * 100))];
...
- (void)drawRect:(NSRect)rect
{
	...
	if(load)
	{
		...
		if(nShapefileType == 1)
		   [self drawShapePoint];
		if((nShapefileType == 3) || (nShapefileType == 5))
...
//                ptToDraw.x = (nEast - rectViewPortM.origin.x)* nZoom  ;
//                ptToDraw.y = (nNorth - rectViewPortM.origin.y)* nZoom ;
